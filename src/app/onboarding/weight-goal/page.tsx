...
/**
 * Onboarding Weight Goal Page (Authenticated)
 * Collects goal weight and target date, then calculates and shows the personalized plan
 */
export default function OnboardingWeightGoalPage() {
    ...
    if (isLoaded && user) {
      const loadUserData = async () => {
        try {
          ...
          // Load existing weight goal if any
          if (data.user?.weightGoal) {
            const goal = data.user.weightGoal;
            if (goal.targetWeight) {
              setGoalWeight(goal.targetWeight.toString());
              setGoalWeightLbs(Math.round(goal.targetWeight * 2.20462 * 10) / 10 + '');
            }
            if (goal.targetDate) {
              const [year, month, day] = goal.targetDate.split('-');
              setDateYear(year);
              setDateMonth(month);
              setDateDay(day);
    ...
    try {
      ...
      await fetch('/api/user/me', {
        ...
        body: JSON.stringify({
          ...
          weightGoal: {
            title: `Lose ${Math.round(currentWeight - goalWeightNum)} kg`,
            startWeight: currentWeight,
            targetWeight: Math.round(goalWeightNum * 10) / 10,
  ...
  if (!isLoaded || !user || isLoadingData || isSaving) {
    return (
